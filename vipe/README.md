...
ViPE relies on PyTorch CUDA wheels. Some newer PyTorch CUDA builds (e.g. `+cu12x`) are compiled only for newer GPU architectures (typically `sm_75+`). If you have an older GPU such as Tesla V100 (`sm_70`), you may see errors like:

`RuntimeError: CUDA error: no kernel image is available for execution on the device`

In that case, install a PyTorch wheel that includes your GPU architecture (often a CUDA 11.8 / `cu118` wheel works well on V100):
