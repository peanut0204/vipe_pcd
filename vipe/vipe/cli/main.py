...
from vipe.utils.device import ensure_cuda_supported_or_explain
...
def infer(video: Path, image_dir: Path, output: Path, pipeline: str, visualize: bool):
    ...
    # Fail fast with an actionable error message when the installed PyTorch CUDA wheel
    # cannot execute kernels on the detected GPU (e.g., V100 sm_70 with cu12.x wheels).
    try:
        ensure_cuda_supported_or_explain()
