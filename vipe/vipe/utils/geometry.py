...
def project_points_to_panorama(
    ...
    Returns:
        ...
        Here u and v is within [0, 1], and d is the actual depth value.
    """
    # Gracefully handle cases where the SLAM map / pointcloud is empty.
    # Some callsites expect an (N, 2/3) tensor even when N==0.
    if xyz.numel() == 0:
        if xyz.dim() >= 1 and xyz.shape[-1] == 3:
            out_shape = (*xyz.shape[:-1], 3 if return_depth else 2)
        else:
            out_shape = (0, 3 if return_depth else 2)
        return torch.empty(out_shape, device=xyz.device, dtype=xyz.dtype)

    # Be tolerant to unexpected shapes (e.g. flattened empty tensors coming from transforms).
    # Prefer preserving the original shape when it already ends with (..., 3).
    if xyz.shape[-1] != 3:
        if xyz.numel() % 3 != 0:
            raise ValueError(f"project_points_to_panorama: expected last dim == 3, got shape {tuple(xyz.shape)}")
        xyz = xyz.reshape(-1, 3)

    depth = torch.linalg.norm(xyz, dim=-1).clamp_min(1e-8)
    u = torch.atan2(xyz[..., 0], xyz[..., 2])  # [-pi, pi]
    v = torch.acos((-xyz[..., 1] / depth).clamp(-1.0, 1.0))  # [0, pi]
