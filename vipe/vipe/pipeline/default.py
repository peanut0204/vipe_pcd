...
class DefaultAnnotationPipeline(Pipeline):
    ...
    def run(self, video_data: VideoStream | MultiviewVideoList) -> AnnotationPipelineOutput:
        ...
        def _outputs_exist(artifact_path: io.ArtifactPath) -> bool:
            ...
            if getattr(self.out_cfg, "save_slam_map", False):
                has_any_output = True
                if not artifact_path.slam_map_path.exists():
                    return False

            return has_any_output

        if self.out_cfg.skip_exists and all(_outputs_exist(ap) for ap in artifact_paths):
