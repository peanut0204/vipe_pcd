...
class TransNetV2(nn.Module):
    def __init__(
        ...
        self.dropout = (
            nn.Dropout(dropout_rate) if dropout_rate is not None else None
        )

        output_dim = (
            ((F * 2 ** (L - 1)) * 4) * 3 * 6
        )  # 3x6 for spatial dimensions
        if use_frame_similarity:
            output_dim += 128
        if use_color_histograms:
            output_dim += 128
...
class FrameSimilarity(nn.Module):
    ...
    def forward(self, inputs):
        ...
        similarities = similarities_padded[
            batch_indices, time_indices, lookup_indices
        ]
...
class ColorHistograms(nn.Module):
    ...
    def compute_color_histograms(frames):
        ...
        frame_bin_prefix = (
            torch.arange(0, batch_size * time_window, device=frames.device)
            << 9
        ).view(-1, 1)
...
def main():
    ...
    parser.add_argument(
        "--weights",
        type=str,
        default="transnetv2-pytorch-weights.pth",
        help="path to TransNet V2 weights, tries to infer the location if not specified",
    )
    parser.add_argument(
        "--output_root", type=str, default="./filter_results/cut_detection"
    )
    parser.add_argument(
        "--video_list",
        type=str,
        default="video_list.csv",
        # default="metadata/video_list_filtered_24k.csv",
    )
    ...
    filelist = filelist[1:]  # remove header

    start = args.start
    end = len(filelist) if args.end == -1 else args.end
    ...
    for file in tqdm(filelist):
        ...
        try:
            input_video = read_video(file_path)
        except Exception as e:
            print(f"Failed to read video {file_path}: {e}")
        else:
            input_video = torch.from_numpy(input_video).unsqueeze(0).to(device)
            try:
                with torch.no_grad():
                ...
                single_frame_pred = (
                    torch.sigmoid(single_frame_pred).cpu().numpy().squeeze()
                )
                all_frame_pred = (
                    torch.sigmoid(all_frame_pred["many_hot"])
                    .cpu()
                    .numpy()
                    .squeeze()
                )
                try:
                    pred = np.stack([single_frame_pred, all_frame_pred], axis=1)
                except np.exceptions.AxisError as e:
                    print(f"Failed to stack predictions for {file_path}: {e}")
                else:
                    # scenes = predictions_to_scenes(single_frame_predictions.cpu().numpy().squeeze(0))
                    # np.savetxt(save_path, scenes, fmt="%d")
                    np.savetxt(save_path, pred, fmt="%.6f")
